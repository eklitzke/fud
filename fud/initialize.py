    ...
    def start(self):
        ...
        while True:
            ...
            try:
                ...
                print >> sys.stderr, 'got event_pairs'
            except Exception, e:
...
class TornadoThread(threading.Thread):
    ...
    def run(self):
        ...
        def stupid_timeout():
            import datetime
            print '-- timeout in fud io_loop %s' % datetime.datetime.now().strftime('%H:%M:%S')
            print 'timeouts are %r, callbacks are %r, handlers are %r, events are %r' % (io_loop._timeouts, io_loop._callbacks, io_loop._handlers, io_loop._events)
